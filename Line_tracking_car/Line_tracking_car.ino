...
void loop() {
  ...
  num3=digitalRead(2); 
   ...
   else if((num3==0)&&num2&&num1) 
   {
    _mright();          //The sensor detected that right car turn left immediately when it meets black line 
    delay(.1);
    while(1){
    num2=digitalRead(10);         //Cycle to judge degree of intermediate sensor,
    if(num2==1)
    {  _mright();                     //If num2==1 does not go to the middle position, continue to turn left.
     delay(.1);}
     else
     break;                      //Detection of num2==0 instructions turned over, out of the loop, detection of three sensors’ statusand then make appropriate action
   }                                       //The following and so on
   ...
   {
    _mBack();          //The sensor detected that right car turn back immediately when it meets black line 
    delay(.1);
    while(1){
    num2=digitalRead(10);         //Cycle to judge degree of intermediate sensor,
    if(num2==1)
